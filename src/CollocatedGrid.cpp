...
double CollocatedGrid::divergenceAt(int i, int j, int k) const
{
    Size3 left, right, front, back, up, down;
    const Size3 gridRes = dataSize();

    left  << std::max(i - 1,(int)0),j,k;
    right << std::min(i + 1,gridRes[0]),j,k;

    down << i, std::max(j - 1, (int) 0), k;
    up << i, std::min(j + 1, gridRes[1]), k;

    back  << i,j,std::max(k - 1,(int)0);
...
{
    int idx = k + j * dataSize()[2] + i * dataSize()[1] * dataSize()[2];
    return data[idx];
}
...
Vec3 CollocatedGrid::curlAt(int i, int j, int k) const
{
    Size3 left, right, front, back, up, down;
