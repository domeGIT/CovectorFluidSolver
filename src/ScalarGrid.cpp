...
Vec3 ScalarGrid::gradientAt(int i, int j, int k) const
{
    ...
    is << std::max(i-1,(int)0),i,std::min(i+1,dimSize[0] - 1);
    js << std::max(j-1,(int)0),j,std::min(j+1,dimSize[1] - 1);
    ks << std::max(k-1,(int)0),k,std::min(k+1,dimSize[2] - 1);
...
double ScalarGrid::at(int i, int j, int k) const
{
...
void ScalarGrid::getData(std::vector<double> *data) const
{
    ...
    unsigned int numElements = dSize[0] * dSize[1] * dSize[2];
...
{
    Size3 dSize = dataSize();
    int numElements = dSize[0] * dSize[1] * dSize[2];
...
void ScalarGrid::foreachIndex(std::function<void(int,int,int)> predicate) const
{
    Size3 size = dataSize();
