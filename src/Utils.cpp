                                     ...
                                     Vec3 *triweights)
{
    ...
    const int iSize = static_cast<int>(res[0]);
    const int jSize = static_cast<int>(res[1]);
    const int kSize = static_cast<int>(res[2]);
                                ...
                                std::vector<double> &output)
{
    ...
    const int iIter = dims[1] * dims[2];
    const int jIter = dims[2];
    const int kIter = 1;
    ...
    for (unsigned int iter = 0; iter < numIters; ++iter)
    {
        ...
        for (int i = 0; i < dims[0]; i++)
        {
            for (int j = 0; j < dims[1]; j++)
            {
                ...
                {
                    int flatIndex = i * dims[1] * dims[2] + j * dims[2] + k;
...
double Utils::cubicInterpolateFromGrid(const ScalarGrid &grid, const Vec3 point)
{
    ...
    std::array<int,4> iPoints = {
        std::max(i - 1, (int)0),
        i,
        std::min(i + 1, sizeI - 1),
        std::min(i + 2, sizeI - 1)
    };
    std::array<int,4> jPoints = {
        std::max(j - 1, (int)0),
        j,
        std::min(j + 1, sizeJ - 1),
        std::min(j + 2, sizeJ - 1)
    };
    std::array<int,4> kPoints = {
        std::max(k - 1, (int)0),
        k,
        std::min(k + 1, sizeK - 1),
        std::min(k + 2, sizeK - 1)
